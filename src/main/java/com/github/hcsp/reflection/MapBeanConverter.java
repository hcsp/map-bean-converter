...
public class MapBeanConverter {
    ...
    private static boolean isGetter(Method method) {
        ...
        return method.getParameterCount() == 0
                && (isFilterMethodName(methodName, "get")
                || isFilterMethodName(methodName, "is"));
    }

    private static boolean isFilterMethodName(String methodName, String fields) {
        return methodName.startsWith(fields)
                && methodName.length() > fields.length()
                && Character.isUpperCase(methodName.substring(fields.length()).charAt(0));
    }
